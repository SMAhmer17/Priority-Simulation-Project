...
def mmdfuncp(request):
         arrival=float(request.GET['arrival']) 
         service=float(request.GET['service'])
         result = munk(arrival,service)

         plt.plot(result["Arrival Times"],label = "Arrival")
         plt.plot(result["Service Times"],label = "Service")
         plt.xlabel('x - axis')
         plt.ylabel('y - axis')
         plt.title('Simulation')
         plt.legend()
         ...
         plot_data1 = base64.b64encode(buffer1.read()).decode() # Encode the plot as a base64 string
         plt.close()
         return render(request, 'realdatashow.html', { 'plot_data1':plot_data1,
            'arrival_times': result["Arrival Times"],
            'service_times': result["Service Times"],
            'start_times': result["Start Times"],
            'end_times': result["End Times"],
            'priorities': result["Priority"],
            'turnaround_times': result["Turnaround Times"],
            'waiting_times': result["Waiting Times"],
            'response_times': result["Response Times"],
        })
